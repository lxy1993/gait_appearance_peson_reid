...
class Mars(object):
    ...
    """
    # root = '/media/work401/documents/MARS-v160809/MARS-evaluation'

    root = "/media/work401/880AA9210AA90CEE/lxy/2020-03-15/CASIA_split_new"
    #root = "/media/tonner/documents/CASIA_split"

    train_name_path = osp.join(root, 'info_v2/train_name.txt')
    test_name_path = osp.join(root, 'info_v2/test_name.txt')
    ...
    ######CASIA_split################
    track_train_info_path = osp.join(root, 'info_4.23/m_train_info.npy')
    track_val_info_path =osp.join(root,"info_4.23/val_info.npy")
    track_test_info_path = osp.join(root, 'info_4.23/test_info.npy')
    query_IDX_path = osp.join(root, 'info_4.23/query_IDX_cl.npy')
    ...
    def __init__(self, min_seq_len=0):
        ...
        track_train = np.load(self.track_train_info_path).astype("int32")
        track_val = np.load(self.track_val_info_path).astype("int32")
        track_test = np.load(self.track_test_info_path).astype("int32")
        query_IDX = np.load(self.query_IDX_path).astype("int32")

        track_query = query_IDX
        ...
        val, num_val_tracklets, num_val_pids, num_val_imgs = \
            self._process_data(train_names, track_val, home_dir='bbox_train', relabel=True, min_seq_len=min_seq_len)
        ...
        num_imgs_per_tracklet = num_train_imgs +num_val_imgs+ num_query_imgs + num_gallery_imgs
        min_num = np.min(num_imgs_per_tracklet)
        max_num = np.max(num_imgs_per_tracklet)
        avg_num = np.mean(num_imgs_per_tracklet)

        num_total_pids = num_train_pids +num_val_pids+ num_query_pids
        num_total_tracklets = num_train_tracklets +num_val_tracklets+ num_query_tracklets + num_gallery_tracklets
        ...
        print("  val      | {:5d} | {:8d}".format(num_val_pids, num_val_tracklets))
    ...
    def _check_before_run(self):
        ...
        if not osp.exists(self.track_train_info_path):
            raise RuntimeError("'{}' is not available".format(self.track_val_info_path))
    ...
    def _process_data(self, names, meta_data, home_dir=None, relabel=False, min_seq_len=0):
        ...
        if relabel: pid2label = {pid: label for label, pid in enumerate(pid_list)}######这个地方是relabelpid，when including pid==-1
        tracklets = []
        num_imgs_per_tracklet = []

        for tracklet_idx in range(num_tracklets):
            data = meta_data[tracklet_idx, ...]
            start_index, end_index, pid, camid,seq_type = data

            #####删减版的数据集#######
            seq = {"1": "bg-01", "2": "bg-02", "3": "cl-01", "4": "cl-02", "5": "nm-01", "6": "nm-02", "7": "nm-03", "8": "nm-04", "9": "nm-05", "10": "nm-06"}
            if str(seq_type) not in list(seq.keys()):
                continue
            ...
            # camid -= 1  # index starts from 0    ####mat文件下标从1开始，npy下标从0开始
            img_names = names[start_index:end_index] ####mat文件下标从1开始，npy下标从0开始

            # make sure image names correspond to the same person
            pnames = [img_name[:3] for img_name in img_names]
            assert len(set(pnames)) == 1, "Error: a single tracklet contains different person images"


            # make sure all images are captured under the same camera
            camnames = [img_name.split("-")[3]for img_name in img_names]
            assert len(set(camnames)) == 1, "Error: images are captured under different cameras!"


            # append image names with directory information
            # img_paths = [osp.join(self.root, home_dir, img_name[:4], img_name) for img_name in img_names]
            img_paths = [osp.join(self.root, home_dir, img_name[:3], img_name) for img_name in img_names]
            if len(img_paths) >= min_seq_len:
                img_paths = tuple(img_paths)

                tracklets.append((img_paths, pid, camid,seq[str(seq_type)]))
